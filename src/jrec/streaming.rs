...
use futures::{future::err, SinkExt, StreamExt, TryStreamExt};
use tokio::io::{self, AsyncReadExt, AsyncSeekExt};
use tokio_util::codec::{Decoder, Encoder, Framed};
use tracing::{debug, error, info};
use winapi::um::winnt::{FILE_SHARE_READ, FILE_SHARE_WRITE};

use crate::utils::recording_manager::{self, RecordingManager};
...
impl StreamFile {
    pub async fn open(path: std::path::PathBuf) -> io::Result<Self> {
...
pub async fn test_stream(
    ...
    tokio::spawn(async move {
        if let Err(e) = handle_request(file, ws_frame, recording_manager).await {
            error!("Error handling request: {:?}", e);
        }
    });
}

async fn reopen(file: StreamFile, seek_position: u64) -> tokio::io::Result<StreamFile> {
    info!("Reopening file");
    let path = file.path.clone();
    drop(file);
    let mut file = StreamFile::open(path).await?;
