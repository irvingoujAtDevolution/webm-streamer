...
export default function WSVideoPlayer({ recordingToPlay }: VideoPlayerProps) {
		...
		const streaming = async () => {
			...
			const pullChunk = async () => {
				...
				if (response?.type === "Chunk") {
					totalSize = response.metadata.total_size;
					offset = response.metadata.offset;
					console.log(`Received chunk: ${offset}/${totalSize}`);
